...
def lfs_pull(file_name):
    result = subprocess.run(['git', 'lfs', 'pull', '--include', file_name, '--exclude', '\'\''], cwd='/root/workspace/models', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(f'LFS pull completed for {file_name} with return code= {result.returncode}')


def lfs_prune():
    result = subprocess.run(['git', 'lfs', 'prune'], cwd='/root/workspace/models', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
